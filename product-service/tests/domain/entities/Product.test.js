...
describe('Product Entity', () => {
    ...
    describe('Product.create()', () => {
        it('should create a valid product with all required fields', () => {
            const product = Product_1.Product.create(validProductDTO);
        ...
        describe('Name validation', () => {
            it('should throw error for empty name', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, name: '' })).toThrow(Product_1.ProductValidationError);
            });
            it('should throw error for name shorter than 3 characters', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, name: 'AB' })).toThrow('Product name must be at least 3 characters');
            });
            it('should throw error for name longer than 200 characters', () => {
                const longName = 'A'.repeat(201);
                expect(() => Product_1.Product.create({ ...validProductDTO, name: longName })).toThrow('Product name cannot exceed 200 characters');
            });
            it('should throw error with correct field identifier', () => {
                try {
                    Product_1.Product.create({ ...validProductDTO, name: '' });
                }
                catch (error) {
        ...
        describe('Description validation', () => {
            it('should throw error for empty description', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, description: '' })).toThrow(Product_1.ProductValidationError);
            });
            it('should throw error for description shorter than 10 characters', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, description: 'Short' })).toThrow('Description must be at least 10 characters');
            });
            it('should throw error for description longer than 2000 characters', () => {
                const longDesc = 'A'.repeat(2001);
                expect(() => Product_1.Product.create({ ...validProductDTO, description: longDesc })).toThrow('Description cannot exceed 2000 characters');
            });
        });
        describe('Price validation', () => {
            it('should throw error for negative price', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, price: -1 })).toThrow('Price cannot be negative');
            });
            it('should throw error for NaN price', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, price: NaN })).toThrow('Price must be a valid number');
            });
            it('should allow zero price (free products)', () => {
                const product = Product_1.Product.create({ ...validProductDTO, price: 0 });
                expect(product.price).toBe(0);
            });
            it('should allow decimal prices', () => {
                const product = Product_1.Product.create({ ...validProductDTO, price: 99.99 });
                expect(product.price).toBe(99.99);
            });
        });
        describe('Category validation', () => {
            it('should throw error for empty category', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, category: '' })).toThrow(Product_1.ProductValidationError);
            });
            it('should throw error for category shorter than 2 characters', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, category: 'A' })).toThrow('Category must be at least 2 characters');
            });
            it('should throw error for category longer than 100 characters', () => {
                const longCategory = 'A'.repeat(101);
                expect(() => Product_1.Product.create({ ...validProductDTO, category: longCategory })).toThrow('Category cannot exceed 100 characters');
            });
        });
        describe('Stock validation', () => {
            it('should throw error for negative stock', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, stock: -1 })).toThrow('Stock cannot be negative');
            });
            it('should throw error for non-integer stock', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, stock: 10.5 })).toThrow('Stock must be a whole number');
            });
            it('should allow zero stock', () => {
                const product = Product_1.Product.create({ ...validProductDTO, stock: 0 });
                expect(product.stock).toBe(0);
            });
        });
        describe('SellerId validation', () => {
            it('should throw error for empty sellerId', () => {
                expect(() => Product_1.Product.create({ ...validProductDTO, sellerId: '' })).toThrow('Seller ID is required');
            });
    ...
    describe('Product.fromPersistence()', () => {
        it('should reconstitute a product from persistence data', () => {
            ...
            const product = Product_1.Product.fromPersistence(persistenceData);
    ...
    describe('Product.update()', () => {
        ...
        it('should update the updatedAt timestamp', () => {
            ...
            expect(updatedProduct.updatedAt.getTime()).toBeGreaterThanOrEqual(originalUpdatedAt.getTime());
        });
        it('should validate updated fields', () => {
            const product = Product_1.Product.create(validProductDTO);
            expect(() => product.update({ name: 'AB' })).toThrow('Product name must be at least 3 characters');
        });
