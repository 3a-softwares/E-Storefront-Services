...
export async function generateAllSampleData(): Promise<GeneratedData> {
    ...
    for (let i = 0; i < 50; i++) {
        ...
        if (coupon) {
            if ((coupon as any).discountType === 'percentage') {
                discount = Math.round(subtotal * ((coupon as any).discount / 100) * 100) / 100;
            } else {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        tickets.push({
            ...
            resolution: status === 'resolved' || status === 'closed' ? TICKET_RESOLUTIONS[i % TICKET_RESOLUTIONS.length] : null,
